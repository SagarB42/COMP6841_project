...
def home():
    ...
    if search_query:
        query = f"SELECT p.id, p.title, p.content, p.timestamp, p.author_id, u.username as author FROM posts p JOIN users u ON p.author_id = u.id WHERE p.visibility = 'public' AND p.title LIKE '%{search_query}%' ORDER BY p.timestamp DESC"
...
def server_status():
    ...
    if 'Mozilla' in user_agent or 'Chrome' in user_agent or 'Safari' in user_agent:
        abort(403)
    return "SERVER HEALTH: OK. DB_CONNECTION: active. API_VERSION: 1.7. INTERNAL USE ONLY. SUPREME_USER : iHeaRtcoMP6841"
